...
target_alias=favor-elf
target_vendor=unknown
target_os=elf
target=favor-unknown-elf

program_transform_name = s&^&favor-elf-&

prefix = /home/emdash/favor-isa/root
exec_prefix = ${prefix}
...
SUBDIRS =  libiberty opcodes bfd readline libbacktrace libdecnumber libctf libsframe binutils gas ld sim etc gnulib
TARGET_CONFIGDIRS = 
# This is set by the configure script to the arguments to use when configuring
...
CC_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-cc

# If GCC_FOR_TARGET is not overriden on the command line, then this
# variable is passed down to the gcc Makefile, where it is used to
# build libgcc2.a.  We define it here so that it can itself be
# overridden on the command line.
GCC_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-gcc
CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-c++
RAW_CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-c++
GFORTRAN_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-gfortran
GOC_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-gccgo
GDC_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-gdc
GM2_FOR_TARGET=$(STAGE_CC_WRAPPER) favor-elf-gm2
DLLTOOL_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/dlltool
DSYMUTIL_FOR_TARGET=favor-elf-dsymutil
LD_FOR_TARGET=$$r/$(HOST_SUBDIR)/ld/ld-new

LIPO_FOR_TARGET=favor-elf-lipo
NM_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/nm-new
OBJDUMP_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/objdump
OBJCOPY_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/objcopy
OTOOL_FOR_TARGET=favor-elf-otool
